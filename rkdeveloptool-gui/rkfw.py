"""
Parser/extractor for Rockchip RKFW-packed firmware images (update.img).

An update.img produced by the Rockchip SDK is a container (magic "RKFW")
holding two blobs:
  - a Loader (the same format rkdeveloptool's `db`/`ul` commands consume
    directly, magic handled internally by rkdeveloptool)
  - an RKAF partition archive (magic "RKAF", the afptool format) holding
    the parameter table plus every partition image (boot, recovery,
    system, ...).

Struct offsets below were verified against the upstream rkdeveloptool
C++ sources (RKImage.h/RKBoot.h, STRUCT_RKIMAGE_HEAD / STRUCT_RKBOOT_HEAD)
by compiling them and printing offsetof()/sizeof(), and against the
afptool.c / rkafp.h reference implementation (RKAFP_MAGIC "RKAF",
struct update_header / update_part) - not guessed from memory. Getting
this wrong writes garbage to the partition table, so every offset here
must trace back to one of those two sources.
"""
import os
import struct
...
# sizeof(STRUCT_RKIMAGE_HEAD) with #pragma pack(1); confirmed via offsetof().
RKFW_HEADER_SIZE = 102
# sizeof(struct update_header) from rkafp.h; confirmed by hand (140 fixed
# fields + 16 * 112-byte update_part entries + 116 reserved bytes = 2048).
RKAF_HEADER_SIZE = 2048
RKAF_MAX_PARTS = 16
RKAF_PART_SIZE = 112
...
class RKFWError(Exception):
    """Raised when an update.img / RKFW / RKAF file can't be parsed safely."""


def rkcrc32(data: bytes, crc: int = 0) -> int:
    """Rockchip's custom CRC-32 variant used by afptool (not zlib crc32)."""
    table = _RKCRC_TABLE
    for byte in data:
...
def parse_rkfw_header(file_path: str) -> RKFWInfo:
    ...
    if file_size < RKFW_HEADER_SIZE:
        raise RKFWError("File is too small to contain an RKFW header")

    with open(file_path, "rb") as f:
        header = f.read(RKFW_HEADER_SIZE)

    if header[0:4] != RKFW_MAGIC:
        raise RKFWError("Not an RKFW-packed image (missing 'RKFW' magic)")
    ...
    if fw_offset <= 0 or fw_size <= 0 or fw_offset + fw_size > file_size:
        raise RKFWError("RKFW header has an invalid firmware (RKAF) offset/size")

    return RKFWInfo(boot_offset=boot_offset, boot_size=boot_size,
                     fw_offset=fw_offset, fw_size=fw_size, file_size=file_size)


def parse_rkaf_header(file_path: str, fw_offset: int) -> RKAFInfo:
    """Parse the embedded RKAF partition archive starting at fw_offset."""
    file_size = os.path.getsize(file_path)
    if fw_offset + RKAF_HEADER_SIZE > file_size:
        raise RKFWError("RKAF header would read past end of file")

    with open(file_path, "rb") as f:
        f.seek(fw_offset)
        header = f.read(RKAF_HEADER_SIZE)
    ...
    version = struct.unpack_from("<I", header, 132)[0]
    num_parts = struct.unpack_from("<I", header, 136)[0]

    if num_parts > RKAF_MAX_PARTS:
        raise RKFWError(f"RKAF header reports {num_parts} parts (max {RKAF_MAX_PARTS})")
    if length <= 0 or fw_offset + length > file_size:
    ...
    return RKAFInfo(model=model, machine_id=machine_id, manufacturer=manufacturer,
...
def extract_part(file_path: str, fw_offset: int, part: RKAFPart, dest_path: str) -> None:
    ...
    trailing 4-byte CRC inside the archive; afptool strips those before
    writing the file out, so we do the same here.
    """
    pos, size = part.pos, part.size
    if part.is_parameter:
        pos += 8
        size -= 12
    if size <= 0:
